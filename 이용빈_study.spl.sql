...
SELECT *
FROM(SELECT TO_CHAR( FIRST_REG_DATE , 'day')
      ,CASE
       WHEN TO_CHAR( FIRST_REG_DATE , 'day')='일요일' THEN COUNT(*)
       WHEN TO_CHAR( FIRST_REG_DATE , 'day')='월요일' THEN COUNT(*)
       WHEN TO_CHAR( FIRST_REG_DATE , 'day')='화요일' THEN COUNT(*)
       WHEN TO_CHAR( FIRST_REG_DATE , 'day')='수요일' THEN COUNT(*)
       WHEN TO_CHAR( FIRST_REG_DATE , 'day')='목요일' THEN COUNT(*)
       WHEN TO_CHAR( FIRST_REG_DATE , 'day')='금요일' THEN COUNT(*)
       WHEN TO_CHAR( FIRST_REG_DATE , 'day')='토요일' THEN COUNT(*)
       END AS 요일
FROM CUSTOMER
...
SELECT *
FROM(
SELECT TO_CHAR( FIRST_REG_DATE , 'day') 
      ,COUNT(*)
FROM CUSTOMER
GROUP BY TO_CHAR( FIRST_REG_DATE , 'day')
ORDER BY 1)
...
----------4-2번 문제 ---------------------------------------------------
-- 남녀 인원수를 출력하시오
--SELECT COUNT(DECODE(SEX_CODE, 'M', '남자', 'F', '여자', '미등록')) as gender 여러개일 경우 한번에 카운터는 불가능
SELECT *
FROM CUSTOMER;
     
       SELECT NVL(a.GENDER,'합계') as gender, cnt
       FROM(SELECT DECODE(SEX_CODE, 'M', '남자', 'F', '여자',NULL, '미등록') as GENDER
     , COUNT(*) as CNT
     FROM CUSTOMER
GROUP BY ROLLUP(DECODE (SEX_CODE, 'M', '남자', 'F', '여자', NULL, '미등록'))) a;

SELECT CASE WHEN SEX_CODE IS NULL AND grouping_id(SEX_CODE) = '1' THEN '합계'
            WHEN SEX_CODE IS NULL AND grouping_id(SEX_CODE) = '0' THEN '미등록'
            WHEN SEX_CODE = 'M' THEN '남자'
            WHEN SEX_CODE = 'F' THEN '여자'
            ELSE TO_CHAR(SEX_CODE)
            END AS GENDER
          , count(*) as CNT
            FROM CUSTOMER
...
SELECT SUBSTR(RESERV_DATE, 5,2) as 월
      , SUM(DECODE(cancel, 'Y', 1, 'N', 0)) as 취소건수
FROM reservation
...
GROUP BY b.item_id, b.product_name -- 이름으로 GROUP BY 걸떄 동명이인이 있을수 있으니
ORDER BY 2 DESC;
...
SELECT SUBSTR(a.reserv_date, 1, 6)as 매출월
    ,SUM(DECODE(b.item_id, 'M001',b.sales,0)) AS SPECTAL_SET
    ,SUM(DECODE(b.item_id, 'M002',b.sales,0)) AS PASTA
FROM reservation a
     ,order_info b
